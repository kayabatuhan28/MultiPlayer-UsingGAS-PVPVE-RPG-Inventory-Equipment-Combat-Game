...
FInv_SpaceQueryResult UInv_InventoryGrid::CheckHoverPosition(const FIntPoint& Position, const FIntPoint& Dimensions) 
{
	...
	UInv_InventoryStatics::ForEach2D(GridSlots, UInv_WidgetUtils::GetIndexFromPosition(Position, Columns),
...
bool UInv_InventoryGrid::CursorExitedCanvas(const FVector2D& BoundaryPos, const FVector2D& BoundarySize, const FVector2D& Location)
{
	...
	if (!bMouseWithinCanvas && bLastMouseWithinCanvas) // Exited
	{
		UnHighlightSlots(LastHighlightedIndex, LastHightlightedDimensions);
		return true;
	}
...
void UInv_InventoryGrid::UnHighlightSlots(const int32 Index, const FIntPoint& Dimensions)
{
	UInv_InventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns, [&](UInv_GridSlot* GridSlot)
	{
		// Suruklenen itemin oldugu gridde item varsa o item hoverlanmis gibi parlar bosa cikan gridlerde unoccupied yani default haline doner
...
FIntPoint UInv_InventoryGrid::CalculateStartingCoordinate(const FIntPoint& Coordinate, const FIntPoint& Dimensions, const EInv_TileQuadrant Quadrant) const
{
	...
	switch (Quadrant)
	{
		...
		case EInv_TileQuadrant::BottomRight:
			...
			break;
		default:
			//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Invalid Quadrant -- Inv_InventoryGrid.Cpp"));
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest, const int32 StackAmountOverride)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		// Can the item fit here? (i.e. is out of grind bounds?)
...
int32 UInv_InventoryGrid::GetStackAmount(const UInv_GridSlot* GridSlot) const
{
	...
	// if we are at a slot doesn't hold the stack count. we must get the actual stack count.
...
void UInv_InventoryGrid::AddStacks(const FInv_SlotAvailabilityResult& Result)
{
	...
	for (const auto& Availability : Result.SlotAvailabilities)
	{
		...
		{
			AddItemAddIndex(Result.Item.Get(), Availability.Index, Result.bStackable, Availability.AmountToFill);
...
{
	// Item Description acıkken bir itemi envanterde tiklayip suruklememiz halinde description acik kaliyordu bu problemi fixlemek icin ekledik.
	...
	// Eger mouse event sag click ise alttaki kisimlara girmesine gerek yok item split islemine gecilir.
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		{
			// Suruklenilen itemin suruklenildigi yerde ayni itemin max stackli hali varsa 2 sinin swaplandigi case
	...
	// Make sure we can swap with a valid item (birden cok itemide kapsamasi halinde swaplanmasini engellemek adina ekledik)
...
void UInv_InventoryGrid::AddItemToIndices(const FInv_SlotAvailabilityResult& Result, UInv_InventoryItem* NewItem)
{
	for (const auto& Availability : Result.SlotAvailabilities)
	{
		AddItemAddIndex(NewItem, Availability.Index, Result.bStackable, Availability.AmountToFill);
		UpdateGridSlots(NewItem, Availability.Index, Result.bStackable, Availability.AmountToFill);
	}
}

void UInv_InventoryGrid::AddItemAddIndex(UInv_InventoryItem* Item, const int32 Index, const bool bStackable,
	...
	UInv_SlottedItem* SlottedItem) const
{
	...
	// 1 0 - Square to the right / 0 1 is the square below it / 1 1 is the square down and to the right one
...
void UInv_InventoryGrid::UpdateGridSlots(UInv_InventoryItem* NewItem, const int32 Index, bool bStackableItem, const int32 StackAmount)
{
	...
	// Pick up yapilan itemin 2 boyutlu grid kaplamasi durumundaki backgroundin occupied setlenmesini saglar.
	UInv_InventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns, [&](UInv_GridSlot* GridSlot)
	{
...
{
	AddItemAddIndex(HoverItem->GetInventoryItem(), Index, HoverItem->IsStackable(), HoverItem->GetStackCount());
...
void UInv_InventoryGrid::ClearHoverItem()
{
	...
	// Show Mouse Cursor after moving and dropping item another index
	ShowCursor();
}
...
void UInv_InventoryGrid::SwapWithHoverItem(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex)
{
	...
	RemoveItemFromGrid(ClickedInventoryItem, GridIndex);
	AddItemAddIndex(TempInventoryItem, ItemDropIndex, bTempIsStackable, TempStackCount);
